...
public:
	...
	data_t	train(const std::vector<data_t>& input, data_t t, double alpha = -1) { return learn(input, t, alpha); }		//	エイリアス
	//	学習、第１引数：入力値、第２引数：教師値
	data_t	calcError(const std::vector<data_t>& input, data_t t);		//	誤差逆伝搬計算のみ for Test
	bool	load(cchar*);
	//bool	save(cchar*);
